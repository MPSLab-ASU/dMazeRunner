...
from dMazeRunner.common.parse_model import *

def main():
    args = parse_arguments()
    layers_models = parse_layers(args)
    ...
    optimizer.set_default_params_optimization(args, params)
    ...
    total_time = 0
    total_layers = 0
    for layers in layers_models:
        for i, layer in enumerate(layers):
            if type(layer) not in [ConvLayer, GemmLayer, DWConvLayer]:
                continue
            layer.env = env
            layer_type = str(type(layer)).split(".")[-1][:-2]
            total_layers += layer.instances
            print("\n")
            print("compiling layer {}: {} ({}), instances: {}".format(layer.index, layer.name, layer_type, layer.instances))
            print(layer)
            # num_evaluations = optimizer.my_get_num_evaluations(layer, params)
            # print("Execution methods evaluated:", num_evaluations)
            # continue
            result, num_evaluations, skip_layer, time_taken = optimizer.auto_optimize(layer, params, args, print_output=True)

            if skip_layer == True:
                continue

            # num_evaluations = optimizer.get_num_evaluations(layer, params)
            optimizer.display_optimized_result(result)
            print("Execution methods evaluated:", num_evaluations)
            print("Time spent in exploration: {} seconds".format(time_taken))
        ...
        if len(layers) > 1:
            print("\n")
            print("="*20, "Summary", "="*20)
            print("Total layers optimized: ", total_layers)
            ...
            print("Search space exploration done in %.2f" % (total_time)  + " seconds.")
...
def parse_arguments():
    ...
    parse_arguments_model(parser)
    optimizer.parse_arguments_mapping(parser)
