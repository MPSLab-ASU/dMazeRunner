...
def get_sample_layer():
    ...
    layer.set_tiling("M", [4, 64, 1, 2])
    layer.set_tiling("C", [32, 1, 4, 4])
...
def test_commonFunctions():
    ...
    assert data_PE_array["W"] == 96
    assert data_PE_array["O"] == 392
...
def test_TotalEnergy():
    ...
    assert abs(energy_SPM_pass[(key_1_dram, key_1_spm)] - 20328317.44) < 1
    assert abs(energy_SPM_pass[(key_2_dram, key_1_spm)] - 20328317.44) < 1
    assert abs(energy_SPM_pass[(key_1_dram, key_2_spm)] - 7218520.32) < 1
    assert abs(energy_SPM_pass[(key_2_dram, key_2_spm)] - 7218520.32) < 1
...
def test_Performance():
    ...
    assert comm_cycles_dict[(key_1_dram, key_2_spm)][0][1] == 392
    assert comm_cycles_dict[(key_2_dram, key_2_spm)][0][1] == 392
    ...
    assert comm_cycles_dict[(key_1_dram, key_2_spm)][1][1] == 4424
    assert comm_cycles_dict[(key_2_dram, key_2_spm)][1][1] == 4424

    # Check cycles for each SPM pass
    dict_cycles_spm_pass = layer.get_Cycles_One_SPM_Pass()
    assert dict_cycles_spm_pass[(key_1_dram, key_1_spm)] == 1057280
    assert dict_cycles_spm_pass[(key_2_dram, key_1_spm)] == 1057280
    assert dict_cycles_spm_pass[(key_1_dram, key_2_spm)] == 935592
    assert dict_cycles_spm_pass[(key_2_dram, key_2_spm)] == 935592
    ...
    assert dict_cycles[(key_1_dram, key_1_spm)] == 1057280*4*32
    assert dict_cycles[(key_2_dram, key_1_spm)] == 1057280*4*32
    assert dict_cycles[(key_1_dram, key_2_spm)] == 935592*4*32
    assert dict_cycles[(key_2_dram, key_2_spm)] == 935592*4*32
