...
def test_TotalEnergy():
    ...
    assert abs(energy_SPM_pass[(key_1_dram, key_1_spm)] - 4997399.04) < 1
    assert abs(energy_SPM_pass[(key_2_dram, key_1_spm)] - 4997399.04) < 1
    assert abs(energy_SPM_pass[(key_1_dram, key_2_spm)] - 6801477.12) < 1
    assert abs(energy_SPM_pass[(key_2_dram, key_2_spm)] - 6801477.12) < 1
    assert abs(energy_SPM_pass[(key_1_dram, key_3_spm)] - 9715887.36) < 1
    assert abs(energy_SPM_pass[(key_2_dram, key_3_spm)] - 9715887.36) < 1
...
def test_Performance():
    layer = get_sample_layer()
    assert layer.computeExecTime_UsefulOps_One_RF_Pass() == 448
    ...
    assert comm_cycles_dict[(key_1_dram, key_3_spm)][1][1] == 6720
    assert comm_cycles_dict[(key_2_dram, key_3_spm)][1][1] == 6720

    # Check cycles for each SPM pass
    dict_cycles_spm_pass = layer.get_Cycles_One_SPM_Pass()
    assert dict_cycles_spm_pass[(key_1_dram, key_1_spm)] == 152320
    assert dict_cycles_spm_pass[(key_2_dram, key_1_spm)] == 152320
    assert dict_cycles_spm_pass[(key_1_dram, key_2_spm)] == 231168
    assert dict_cycles_spm_pass[(key_2_dram, key_2_spm)] == 231168
    assert dict_cycles_spm_pass[(key_1_dram, key_3_spm)] == 467712
    assert dict_cycles_spm_pass[(key_2_dram, key_3_spm)] == 467712
    ...
    assert dict_cycles[(key_1_dram, key_1_spm)] == 152320*4*32
    assert dict_cycles[(key_2_dram, key_1_spm)] == 152320*4*32
    assert dict_cycles[(key_1_dram, key_2_spm)] == 231168*4*32
    assert dict_cycles[(key_2_dram, key_2_spm)] == 231168*4*32
    assert dict_cycles[(key_1_dram, key_3_spm)] == 467712*4*32
    assert dict_cycles[(key_2_dram, key_3_spm)] == 467712*4*32
