...
def get_sample_layer():
    ...
    layer.set_tiling("N", [1, 1, 1, 4])
    layer.set_tiling("M", [4, 2, 8, 8])
...
def test_commonFunctions():
    ...
    assert data_RF["I"] == 24
    assert data_RF["W"] == 192
    assert data_RF["O"] == 8

    data_PE_array = layer.get_data_allocated_PE_array()
    assert data_PE_array["I"] == 576
    assert data_PE_array["W"] == 9216
...
def test_TotalEnergy():
    ...
    assert abs(energy_SPM_pass[(key_1_dram, key_1_spm)] - 21650003.2) < 1
    assert abs(energy_SPM_pass[(key_2_dram, key_1_spm)] - 21650003.2) < 1
    assert abs(energy_SPM_pass[(key_1_dram, key_2_spm)] - 5838515.2) < 1
    assert abs(energy_SPM_pass[(key_2_dram, key_2_spm)] - 5838515.2) < 1
    assert abs(energy_SPM_pass[(key_1_dram, key_3_spm)] - 3460787.2) < 1
    assert abs(energy_SPM_pass[(key_2_dram, key_3_spm)] - 3460787.2) < 1
...
def test_Performance():
    ...
    assert layer.computeExecTime_UsefulOps_One_RF_Pass() == 192

    cycles = layer.calculate_Cycles_DataDistribution_NOC()
    assert cycles[layer._I.name] == 576
    assert cycles[layer._W.name] == 9216
    ...
    assert cycles[layer._O.name] == 1280
    ...
    assert comm_cycles_dict[(key_1_dram, key_1_spm)][0][1] == 9216
    assert comm_cycles_dict[(key_2_dram, key_1_spm)][0][1] == 9216
    ...
    assert comm_cycles_dict[(key_1_dram, key_1_spm)][1][1] == 1472
    assert comm_cycles_dict[(key_2_dram, key_1_spm)][1][1] == 1472
    assert comm_cycles_dict[(key_1_dram, key_2_spm)][1][1] == 1472
    assert comm_cycles_dict[(key_2_dram, key_2_spm)][1][1] == 1472
    assert comm_cycles_dict[(key_1_dram, key_3_spm)][1][1] == 1472
    assert comm_cycles_dict[(key_2_dram, key_3_spm)][1][1] == 1472

    # Check cycles for each SPM pass
    dict_cycles_spm_pass = layer.get_Cycles_One_SPM_Pass()
    assert dict_cycles_spm_pass[(key_1_dram, key_1_spm)] == 1047424
    assert dict_cycles_spm_pass[(key_2_dram, key_1_spm)] == 1047424
    assert dict_cycles_spm_pass[(key_1_dram, key_2_spm)] == 321664
    assert dict_cycles_spm_pass[(key_2_dram, key_2_spm)] == 321664
    assert dict_cycles_spm_pass[(key_1_dram, key_3_spm)] == 217984
    assert dict_cycles_spm_pass[(key_2_dram, key_3_spm)] == 217984
    ...
    assert dict_cycles[(key_1_dram, key_1_spm)] == 1047424*4*32
    assert dict_cycles[(key_2_dram, key_1_spm)] == 1047424*4*32
    assert dict_cycles[(key_1_dram, key_2_spm)] == 321664*4*32
    assert dict_cycles[(key_2_dram, key_2_spm)] == 321664*4*32
    assert dict_cycles[(key_1_dram, key_3_spm)] == 217984*4*32
    assert dict_cycles[(key_2_dram, key_3_spm)] == 217984*4*32
