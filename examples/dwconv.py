...
import argparse
import json
from dMazeRunner.dataflow import DWConvLayer
from dMazeRunner.common import expr_parameters


def get_sample_layer(env):
    args = {
        #  MobileNetV2 Depthwise Separable Convolution layer
        "name": "test_features.15.conv.3_from_mobinenet_v2",
        "channels": 960,
        "kernel_size": "[3, 3]",
        "padding": "[1, 1]",
        "strides": "[1, 1]",
        "output_shape": [1, 960, 7, 7],
        "input_shape": [1, 960, 7, 7],
        "batch_size": 4,
    }

    layer = DWConvLayer(env, **args)
    layer.set_tiling("N",  [4, 1, 1, 1])
    layer.set_tiling("M",  [1, 1, 1, 1])
    layer.set_tiling("C",  [8, 4, 6, 5])
